...
class load(object):
    ...
    def fit(self, use_ultranest = False, use_dynesty = False, dynamic = False, dynesty_bound = 'multi', dynesty_sample='rwalk', dynesty_nthreads = None, \
            n_live_points = 1000, ecclim = 1., delta_z_lim = 0.5, pl = 0.0, pu = 1.0, require_planet_periods_ordered = 0.0):
        """
        ...
        return fit(self, use_ultranest = use_ultranest, use_dynesty = use_dynesty, dynamic = dynamic, dynesty_bound = dynesty_bound, dynesty_sample = dynesty_sample, \
                   ...
                   pl = pl, pu = pu, require_planet_periods_ordered=require_planet_periods_ordered)
...
class fit(object):
    ...
    :param require_planet_periods_ordered: (optional, float)
        When multiple planets are defined, and this option is True,
        then only solutions with P_i < P_(i+1) * f are allowed.
        where f = require_planet_periods_ordered.
        To disable, set to zero. To enable, set to 1.0 or slightly larger.
    """
        ...
        for pname in self.model_parameters:
            if self.data.priors[pname]['distribution'] != 'fixed':
                ...
                if pname.startswith('P_'):
                    if not cube[pcounter] > last_period * self.require_planet_periods_ordered:
                        return -1e102
                    last_period = cube[pcounter]
    ...
    def __init__(self, data, use_ultranest = False, use_dynesty = False, dynamic = False, dynesty_bound = 'multi', dynesty_sample='rwalk', dynesty_nthreads = None, \
                       ...
                       require_planet_periods_ordered = 0.):
        ...
        self.require_planet_periods_ordered = float(require_planet_periods_ordered)
        ...
        if self.use_ultranest:
            ...
            results = sampler.run(frac_remain=0.5, min_num_live_points=self.n_live_points, max_num_improvement_loops=1)
